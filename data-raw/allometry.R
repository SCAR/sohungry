...
build_allometry_df <- function() {
    ...
    x <- tribble(~taxon_name,~aphia_id,~equation,~input,~return,~units,~N,~notes,~reference,
    ...
    x <- bind_rows(x,tribble(~taxon_name,~aphia_id,~equation,~input,~return,~units,~N,~notes,~reference,
